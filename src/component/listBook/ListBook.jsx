...
//   useEffect(() => {
//     const storedBooks = JSON.parse(localStorage.getItem(tab === 'read' ? 'readbook' : 'wishlist')) || [];
...
function ListedBooksPage() {
  const [sortBy, setSortBy] = useState('rating'); // Default sorting option
  const [books, setBooks] = useState([]);
  const [tab, setTab] = useState('read');

  useEffect(() => {
    // Fetch books from local storage based on the selected tab
